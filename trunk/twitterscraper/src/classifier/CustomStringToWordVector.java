 ...
 * @see Stopwords
 */
public class CustomStringToWordVector extends Filter implements UnsupervisedFilter, OptionHandler {
	...
	public FastVector getForcedAttributes() {
		return forcedAttributes;
	}

	public void setForcedAttributes(FastVector forcedAttributes) {
	 ...
	 */
	public CustomStringToWordVector() {
	}
	...
	public Enumeration listOptions() {
		...
		result.addElement(new Option("\tConvert all tokens to lowercase before " + "adding to the dictionary.", "L", 0,
				"-L"));

		result.addElement(new Option("\tIgnore words that are in the stoplist.", "S", 0, "-S"));

		result.addElement(new Option("\tThe stemmering algorihtm (classname plus parameters) to use.", "stemmer", 1,
				"-stemmer <spec>"));
	 ...
	 *            assigned).
	 */
	public CustomStringToWordVector(int wordsToKeep) {
	...
	public String doNotOperateOnPerClassBasisTipText() {
		return "If this is set, the maximum number of words and the "
				+ "minimum term frequency is not enforced on a per-class "
				+ "basis but based on the documents in all the classes " + "(even if a class attribute is set).";
	}
	...
	public static void main(String[] argv) {
		runFilter(new CustomStringToWordVector(), argv);
	}
}

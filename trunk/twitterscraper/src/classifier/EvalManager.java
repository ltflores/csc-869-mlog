...
public class EvalManager {
	 ...
	 * 	classifier options, comma delimited.
	 * 	in	
	 * 	k
	 *  classifier
	 *  loader
	...
	private void writeHeader(CsvWriter csvOutput) {
		...
		String[] header = { "in","classifier","loader","tokenizer","stemmer","k","features",
				"ngrammin",	"stopwords","evaluate","modelFileName","save",
				// Output results
	...
	private void eval(EvalRecord params){
		...
		Instances tweets = tweetclass.loadTweets(params.getIn(), 
				params.getLoader(), params.getTokenizer(), params.getStemmer(),
				params.isStopwords(), params.getFeatures(), params.getNgrammin());
        
        if (params.isEvaluate()){
        	tweetclass.evaluateClassifier(tweets, params.getClassifier(), params.getK());
        } else {
        ...
        if(params.getModelFileName() == null) {
    		...
    		System.out.println("Using -save=" + params.getModelFileName());
    		tweetclass.saveClassifier(tweets, params.getClassifier(), params.getModelFileName());
    	}
